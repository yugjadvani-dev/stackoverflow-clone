...
export default function Question() {
  ...
  const filteredQuestions = useMemo(() => {
    ...
    if (filter.status !== "all") {
      filtered = filtered.filter((q) => q.answers >= Number(filter.status));
    }

    if (filter.sort === "newest") {
      filtered = filtered.sort(
        (a, b) => Number(new Date(b.askedAt)) - Number(new Date(a.askedAt))
      );
    } else if (filter.sort === "oldest") {
      filtered = filtered.sort(
        (a, b) => Number(new Date(a.askedAt)) - Number(new Date(b.askedAt))
      );
